...
You can also implement Java-like inheritance with `Object.create` in which methods will be inherited, and can be overriden as well.  
Alternatively, you can use the `class` keyword, and extend the class with `class B extends A`. For large prototype chains, avoiding iterating all over all the attributes of the object, but instead lookup the property with `hasOwnProperty`.
